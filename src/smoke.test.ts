...
describe('smoke: end-to-end editor flow', () => {
  ...
  it('propagates model selection to sidebar before save', async () => {
    const configStore = new ConfigStore(tmpDir);
    const profileStore = new ProfileStore(configStore);
    const executor = stubExecutor([{ modelId: 'openai/gpt-4' }]);
    const modelDiscovery = new ModelDiscovery(executor as never, extensionPath);
    const treeProvider = new AgentModelTreeProvider(configStore, profileStore);
    ...
    sendToWebview('ready');
    await new Promise((r) => setTimeout(r, 10));
    sendToWebview('modelChanged', { modelId: 'openai/gpt-4' });
    await new Promise((r) => setTimeout(r, 10));

    const group = treeProvider.getChildren()!.find((g) => g.group === 'agents')!;
    const sisyphus = treeProvider.getChildren(group).find((l) => l.nodeName === 'sisyphus')!;
  ...
  it('clears preview on panel dispose', async () => {
    const configStore = new ConfigStore(tmpDir);
    const profileStore = new ProfileStore(configStore);
    const executor = stubExecutor([{ modelId: 'openai/gpt-4' }]);
    const modelDiscovery = new ModelDiscovery(executor as never, extensionPath);
    ...
    const { panel, sendToWebview, disposeListeners } = makeMockWebviewPanel();
    vi.mocked(vscode.window.createWebviewPanel).mockReturnValue(panel as unknown as import('vscode').WebviewPanel);
    ...
    const group = treeProvider.getChildren()!.find((g) => g.group === 'agents')!;
    const sisyphus = treeProvider.getChildren(group).find((l) => l.nodeName === 'sisyphus')!;
    expect(sisyphus.label).toBe('sisyphus \u2192 old/model');
