<script>
  ...
  function targetKey(target) {
    ...
    if (t.type === 'agent' || t.type === 'category') {
      return `${t.type}:${t.name}:${t.profile ?? ''}`;
    }
    return '';
  }
  ...
  function persist() { persistEditorState(getApi(), currentTarget(), { kind: entityKind, name: entityName, profile: entityProfile, routing: routingSnapshot(), dirty: formDirty, values: serializeValues() }); }
  function restorePersisted(target) { return restoredEditorState(getApi(), target); }
  ...
  function processInit(data) {
    ...
    if (!key) return;
    if (key === lastInitKey) {
      if (formDirty && matchesCurrentTarget(data)) setDirty(true);
      return;
    }
    lastInitKey = key;
    const persisted = restorePersisted({ type: data.type, name: data.name, profile: data.profile ?? null });
    if (persisted?.routing && persisted?.dirty) {
      ...
      initialized = true;
      const target = currentTarget();
      if (target) postMessage({ command: 'dirtyState', target, dirty: true });
    } else {
      applyInit(data);
    }
  }
  ...
  function handleMessage(event) {
    ...
    if (message.command === 'init') processInit(message);
