...
- Task 11: Extracting the structured editor from `App.svelte` into `AgentFormEditor.svelte` preserves behavior best by keeping the original `onMount`/`applyInit`/`restorePersisted` flow and adding `processInit` to restore target-keyed dirty drafts before applying a new host init.
- Task 11: The `ProfileJsonEditor` must not re-apply host `saved`/`error` messages for different profile targets; using `isSameTarget` against the current `profileJson` target and ignoring mismatches keeps the UI stable when switching targets.
- Task 11: `happy-dom` tests need to inspect the versioned envelope state (`{ v: 1, targets: { '<key>': ... } }`) rather than the old flat object; a small `lastStateForTarget` helper avoids brittle assertions.
- Task 11: Re-persisting a dirty restored draft should re-emit the `dirtyState` message so the host title updates correctly after panel re-creation or target switching.
